...
function LoyaltriMobileApplication() {
  ...
  return (
    <article className="relative flex flex-col w-[380px] h-[250px] min-w-[320px] min-h-[250px] bg-white rounded-2xl border-violet-600 border-opacity-20 overflow-hidden">
      ...
      {/* Modal for additional user request */}
      <ModalAnt
        isVisible={isModalOpen}
        onClose={() => setIsModalOpen(false)}
        showOkButton={false} // Hide the default Ok button since we are using custom buttons
        showCancelButton={false} // Hide the default Cancel button
        showTitle={false} // Remove the title as we use custom content
        centered={true}
        padding="8px"
        customButton={
          <ActionButtons /> // Using ActionButtons component for Request and Make Payment buttons
        }
      >
